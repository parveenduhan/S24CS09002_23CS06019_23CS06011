...
#include<queue>
using namespace std;
    const char *S2_IP = "172.31.20.142"; // System2 IP
    const char *S3_IP = "172.31.26.40"; //System 3 IP
    const int S2_PORT = 8080;
    const int S3_PORT = 8082;
...
public:
    ...
    int setupSock(int conn_type)
    {
        ...
        if (conn_type == 1)
        {
            ...
            address.sin_addr.s_addr = inet_addr(S2_IP);
...
int main()
{
    ...
    In this case, this system(S1) is client for S2 and server for S3*/
    ...
    thread receiverThread1([&localSystem, S2_sock]() { localSystem.receiveEvents(S2_sock); }); //separate thread for receiving messages from S2

    thread receiverThread2([&localSystem, S3_sock]() { localSystem.receiveEvents(S3_sock); }); //separate thread for receiving messages from S3

   /*
        Perform local and send Events here:
    
        For e.g. localSystem.sendEvent(S2_sock)
